...
def gen_ptc(img, depth, pinehole_cam, extrinsic):
    ...
    # print(min(depth_img), max(depth_img))
    ...
    rgbd_img = o3d.geometry.RGBDImage.create_from_color_and_depth(color_img, depth_img,
                                                                  depth_scale=1.0, depth_trunc=12.0,
                                                                  convert_rgb_to_intensity=False)

    ptc = o3d.geometry.PointCloud.create_from_rgbd_image(rgbd_img, intrinsic=pinehole_cam) #, extrinsic=extrinsic)
...
def get_partial_ped_ptc(seq, ptc_dict, bbox_dict):
    partial_ptc_dict = {}
    keys_for_delete = []
    # green, blue, purple, yellow, orange
    colors = [[72, 179, 212], [102, 255, 255], [204, 153, 255], [231, 165, 87], [167, 97, 79], [255, 153, 153]]
    # colors = [[102, 255, 255], [231, 165, 87], [255, 153, 153], [72, 179, 212], [204, 153, 255], [167, 97, 79]]
    # colors = [[72,179,212], [231,165,87], [71,25,15], [80,106,133], [167,97,79], [160,82,45]]
    # colors = [[220, 144, 108], [227, 217, 122], [204, 153, 255], [183, 224, 176], [185, 139, 103], [120, 117, 154]]
    # colors = [[220, 220, 220]] * 6
    # colors = [[51, 153, 51], [255, 144, 33], [51, 153, 51], [170, 113, 255], [102, 0, 255]]
    # colors = [[204, 153, 255]] * 6
    colors = [[3, 72, 133]] * 6
    # for iccv seq 10192, 40
    # colors_specific1, colors_specific2 = [231, 130, 67], [204, 153, 255]
    # colors = [[195, 102, 102], [102, 255, 255], [204, 153, 255], [231, 165, 87], [167, 97, 79], [255, 153, 153]]
    colors_specific1, colors_specific2 = [236, 134, 0], [255, 85, 85]
    if '10192' in seq:
        colors[1] = colors_specific1
        colors[2] = colors_specific2
    # colors = [[80,106,133], [204, 153, 255], [231, 165, 87], [167, 97, 79], [255, 153, 153]]
    # for iccv seq 16958, 120
    if '16958' in seq:
        colors[4] = colors_specific1

    # for iccv seq '10210', 24
    if '10210' in seq:
        colors[0] = colors_specific1

    # for iccv seq 11650, 16
    if '11650' in seq:
        colors[0] = colors_specific1
    ...
    for key_ptc in ptc_dict.keys():
        ...
        # for key_bbox in bbox_dict.keys():
        #     print(key_bbox)
        ...
        for idx, mask in enumerate(masks):
            ...
            # z_color_adj = np.expand_dims(ptc_points[mask][:, -1], axis=1).repeat(3, axis=-1)
            z_color_adj = ptc_colors[mask]
            # print(z_color_adj)
            if len(z_color_adj) == 0:
                continue
            z_color_adj = (z_color_adj - np.min(z_color_adj)) / (np.max(z_color_adj) - np.min(z_color_adj))
            # print(z_color_adj)

            if mask_names[idx] in ['740170', '257097', '634451', '712489', '120592', '262934']:
                colors[idx] = colors_specific1

            ptc_partial.colors = o3d.utility.Vector3dVector((np.asarray([colors[idx]]) / 255).repeat(bg_p, axis=0) \
                                                            + (z_color_adj / 255) * 50)
            ...
            # ptc_partial = ptc_partial.voxel_down_sample(voxel_size=0.05)

            partial_ptc_dict[f'pcd_{mask_names[idx]}_{key_ptc[-8:-4]}.pcd'] = ptc_partial

            # if colors[idx] == colors_specific1:
            #     o3d.io.write_point_cloud('17610_40_partial_ped.pcd', ptc_partial)

        mask_all = np.logical_or.reduce(np.array(masks))
        bg_p = np.sum(~mask_all)
        ptc_background = o3d.geometry.PointCloud()
        ptc_background.points = o3d.utility.Vector3dVector(ptc_points[~mask_all])
        dist = np.sqrt(
            ptc_points[~mask_all][:, 0] ** 2 + ptc_points[~mask_all][:, 0] ** 2 + ptc_points[~mask_all][:, 2] ** 2)
        dist = np.expand_dims(dist, axis=1)
        dist_adj = ((dist - np.min(dist)) / (np.max(dist) - np.min(dist))).repeat(3, axis=-1)
        # print(dist_adj)
        ptc_background.colors = o3d.utility.Vector3dVector(ptc_colors[~mask_all] / 10 \
                                                           + (dist_adj / 255) * 200)

        indices = np.random.permutation(len(ptc_points[~mask_all]))[:int(len(ptc_points[~mask_all]) / 10)]
        # ptc_background = ptc_background.select_by_index(indices)
...
def process_sequence_ptc(mta_path, seq, dst=None, specific_frame=None, generate='11ps'):
    ...
    if generate[2] == 'p':
        ptc_dict, bbox_dict = get_partial_ped_ptc(seq, ptc_dict, bbox_dict)

    if generate[3] == 'v':
        if specific_frame:

            return list(ptc_dict.values()) + list(bbox_dict.values()), cam_param
        else:
            return ptc_dict, bbox_dict, cam_param
...
if __name__ == '__main__':
    mta_path = r'E:\MTA-multi'
    seq = 'seq_00007522'

    # process_sequence_ptc(mta_path, seq, specific_frame=10, generate='11fv')

    for idx, seq in enumerate(os.listdir(mta_path)):
        if idx < 0:
            continue
        log_path = os.path.join(mta_path, seq, 'log.txt')
        try:
            process_sequence_ptc(mta_path, seq, dst=r'E:\MTA-ptc')  # specific_frame=10,

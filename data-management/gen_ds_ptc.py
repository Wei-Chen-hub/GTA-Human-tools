    ...
    for z in z1, z2:
        for x in x1, x2:
            for y in y1, y2:
                ...
                bb_pts.append((temp[0] * 6.66, temp[1] * 6.66, temp[2] * 6.66))
...
def generate_pt_cloud(image_c, image_d, log_path, ped_id_df, down_scale=None, visible_length=8, mask_input=False,
    ...
    if mask_input:
        ...
        mask = mask_depth
        bbox3d_pts, coordinates, bbox = None, None, None
    ...
    if visualize:
        ...
        # print(pcd)
        look_at = np.average(np.asarray(bbox.points), 0)
        return pcd, bbox, look_at


def gen_seq_ptc(mta_p, seq, dest):
    for image in os.listdir(os.path.join(mta_p, seq)):
        ...
        for ped_id in ped_id_list:
            ...
            pcd_ped, bbox_ped, look_at_ped = generate_pt_cloud(image_c, image_d, log_path, ped_id_df, down_scale=None,
                                                               visible_length=8,
                                                               mask_input=True,
                                                               visualize=False)
            pcd.append(pcd_ped)
...
def process_some_sequence():
    ...
    mta_path = r'\\DESKTOP-LV0VM5K\gta_multi_ssd2\MTA-multi-p\pending_upload_multi'
    ...
    for index in range(seq_num):
        try:
            print(seqs[index])
            x = threading.Thread(target=gen_seq_ptc(mta_path, seqs[index], dest))
            x.start()

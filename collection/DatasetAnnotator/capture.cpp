...
{
	std::string depth_name = std::string("depth_") + img_seq + std::string(".raw");
	std::string stencil_name = std::string("stencil_") + img_seq + std::string(".raw");
	std::string color_name = std::string("color_") + img_seq + std::string(".raw");

	if(depth_path == "test")
	{
		auto f = fopen(depth_name.c_str(), "wb");
		void* buf;
		int size = export_get_depth_buffer(&buf);
		fwrite(buf, 1, size, f);
		fclose(f);
		f = fopen(stencil_name.c_str(), "wb");
		size = export_get_stencil_buffer(&buf);
		fwrite(buf, 1, size, f);
		fclose(f);
		// f = fopen(color_name.c_str(), "wb");
		...
		try {
			BOOL make_dir = _mkdir(depth_path.c_str());
		}
		catch (const std::exception& e) {
			throw;
		}
		depth_name = depth_path + std::string("\\") + std::string("depth_") + img_seq + std::string(".raw");
		stencil_name = depth_path + std::string("\\") + std::string("stencil_") + img_seq + std::string(".raw");
		// color_name = depth_path + std::string("\\") + std::string("color_") + img_seq + std::string(".raw");
		...
		if (capture_stencil) {
			auto f = fopen(stencil_name.c_str(), "wb");
			void* buf;
